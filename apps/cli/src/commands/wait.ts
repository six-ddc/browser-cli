...
export const waitCommand = new Command('wait')
  ...
  .action(async (selectorOrMs: string | undefined, opts: { timeout: string; hidden?: boolean; url?: string; text?: string; load?: string | boolean; fn?: string }, cmd: Command) => {
    ...
    if (opts.load !== undefined) {
      const load = (typeof opts.load === 'string' ? opts.load : 'load') as 'load' | 'domcontentloaded' | 'networkidle';
      await sendCommand(cmd, {
        action: 'wait',
        params: { load, timeout },
      });
