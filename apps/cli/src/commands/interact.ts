...
import { addTargetOptions, positionFrom, sendCommand, type TargetOptions } from './shared.js';

export const clickCommand = addTargetOptions(
  new Command('click')
  ...
  async (
    selector: string,
    opts: TargetOptions & { button: string; debugger?: true },
    cmd: Command,
  ) => {
...
export const dblclickCommand = addTargetOptions(
  new Command('dblclick')
    .description('Double-click an element (--debugger for CDP isTrusted events)')
    .argument('<selector>', 'CSS selector or @ref')
    .option('--debugger', 'Use Chrome DevTools Protocol for trusted events (isTrusted=true)'),
).action(async (selector: string, opts: TargetOptions & { debugger?: true }, cmd: Command) => {
  await sendCommand(cmd, {
    action: 'dblclick',
    params: {
      selector,
...
export const hoverCommand = addTargetOptions(
  new Command('hover')
...
export const fillCommand = addTargetOptions(
  new Command('fill')
  ...
  async (
    selector: string,
    value: string,
    opts: TargetOptions & { debugger?: true },
    cmd: Command,
  ) => {
    await sendCommand(cmd, {
      action: 'fill',
...
export const typeCommand = addTargetOptions(
  new Command('type')
  ...
  async (
    selector: string,
    text: string,
    opts: TargetOptions & { delay: string; debugger?: true },
    cmd: Command,
  ) => {
...
export const pressCommand = addTargetOptions(
  new Command('press')
    .description(
  ...
  async (
    key: string,
    opts: TargetOptions & { selector?: string; debugger?: true },
    cmd: Command,
  ) => {
    await sendCommand(cmd, {
      action: 'press',
...
export const clearCommand = addTargetOptions(
  new Command('clear')
...
export const focusCommand = addTargetOptions(
  new Command('focus')
